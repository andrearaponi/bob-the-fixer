...
# Install Bob's agent skills (skills/*) for every detected coding agent, each
# through its native mechanism. Best-effort: only runs from a checkout
# (curl-bootstrap mode has no skills directory) and never fails the install.
